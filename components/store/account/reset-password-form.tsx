...
export function ResetPasswordForm() {
  ...
  // O /auth/callback já validou o link (via ?code= ou ?token_hash=) e criou a
  // sessão de recuperação em cookies. Aqui confirmamos que há um usuário na
  // sessão de recovery antes de liberar o formulário.
  useEffect(() => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Encerra a sessão temporária de recuperação para que o cliente faça
      // login com a nova senha (não redirecionamos para a home).
    ...
    return (
      <div className="mx-auto max-w-md px-4 py-10 lg:py-16">
        <div className="flex flex-col items-center gap-4 rounded-3xl border border-border bg-card p-8 text-center shadow-sm">
          ...
          <p className="text-pretty text-sm leading-relaxed text-muted-foreground">
            Sua senha foi redefinida. Você já pode entrar na sua conta com a
            nova senha. Redirecionando para o login...
          </p>
          <Link
            href="/conta"
            className="mt-2 inline-flex h-12 items-center justify-center rounded-full bg-primary px-6 text-sm font-bold text-primary-foreground transition-colors hover:bg-primary/90"
